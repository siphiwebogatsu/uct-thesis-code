...
```{r}
all_irm_years = read_delim("zaf-nt-irm-v1.2.txt")
```
...
all_irm_years_wide <- all_irm_years_wide |>
mutate(
dat_start = parse_date(fix_dates(dat_start)),
# derive start year cleanly from the parsed date
dat_start_yr = lubridate::year(dat_start),
# patch a rare malformed completion year
dat_proj_compl_yr = dplyr::case_when(
dat_proj_compl_yr %in% c("2615", 2615) ~ 2015L,
TRUE ~ suppressWarnings(as.integer(dat_proj_compl_yr))
)
)
```
...
```{r}
write_delim(all_irm_years, "zaf-nt-irm-v1.3.txt")
```
